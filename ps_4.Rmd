...
The first response of Rep came `r elections %>% filter(response %in% c("Rep", "Dem")) %>% group_by(response) %>% select(response, timestamp) %>% summarize(first = min(timestamp)) %>% summarize(diff = round(first[1] - first[2]))` minutes (rounded to the nearest minute) before the first response of Dem.
...
elections %>% 
  ...
  count(response) %>%
  ggplot(aes(x = response, y = n, fill = ager)) + geom_col(position = "dodge2") +
